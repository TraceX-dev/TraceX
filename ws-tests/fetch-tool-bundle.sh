...
# Extract the version of tracex/transactor from the docker-compose file
VERSION=$(grep "image: tracex/transactor:" "$DOCKER_COMPOSE_PATH" | head -1 | sed -E 's/.*image: tracex\/transactor:([^[:space:]]+).*/\1/')
...
echo "Fetching tracex/tool:$VERSION..."

# Pull the tool image with the same version
docker pull "tracex/tool:$VERSION"

# Create a temporary container from the image
CONTAINER_ID=$(docker container create "tracex/tool:$VERSION")
