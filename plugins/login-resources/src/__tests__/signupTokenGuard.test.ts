...
describe('SignupForm token guard (#10518)', () => {
  ...
  it('calls logIn and redirects when token is an empty string (malformed response)', async () => {
    // The server never sends token: '' in practice, but document the guard
    // behaviour: `!= null` passes an empty string through to logIn().
    // If this becomes a real concern the guard should be changed to `result.token`.
