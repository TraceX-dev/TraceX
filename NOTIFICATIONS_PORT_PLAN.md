...
Изначально подтверждали: тащим оба куска (индикатор + логотипы), десктопный tray/dock-бейдж — не в этот раз. **Индикатор реализован** (см. раздел 5 «Статус реализации», коммиты `c22ba12415`/`d0c6ac802a`/`99e3d8aa2e`, своей архитектурой — не через `pulse`/`services/notifications`, см. ниже почему). **Логотипы — решили не портировать**, см. следующий подраздел.

### Логотипы воркспейсов — решили НЕ портировать

При разборе оказалось, что показ логотипа в свитчере воркспейсов требует отдельного публичного HTTP-эндпоинта на account-service (`GET /account/workspace/:uuid/logo`), который сам раздаёт байты картинки в обход обычной раздачи файлов через `getFileUrl`/front-сервер. Причина: `getFileUrl()` всегда резолвит URL относительно **текущего** подключённого воркспейса (`getCurrentWorkspaceUuid()`), а front-сервер (`server/front/src/index.ts`) жёстко сверяет workspace в токене с workspace в пути запроса и отклоняет чужой — токен воркспейса A не может забрать файл воркспейса B. Значит, чтобы показать логотип воркспейса, который пользователь ещё не открывал, нужен отдельный кросс-воркспейсный канал раздачи файлов именно через account-service — то есть ещё один эндпоинт со своей моделью авторизации/кэширования, который живёт отдельно от обычного файлового стораджа.

Проверили дальнейшую историю `intabia/develop` после коммита с логотипами (`2bb6533556`) — и нашли, что сама команда intabia позже (`017bd9eec1`, «Remove logo from account») **полностью откатила** эту функциональность: убрали колонку `workspace.logo`, RPC `updateWorkspaceLogo`, сам account-service эндпоинт (~99 строк), и в `SelectWorkspaceMenu.svelte`/`Logo.svelte` вернулись к состоянию без логотипов в свитчере — только точка-индикатор непрочитанного (то есть к тому состоянию, которое мы уже реализовали). Раз даже автор фичи счёл цену (отдельный публичный файловый эндпоинт в account-service) не стоящей результата (картинка в списке воркспейсов) и откатил это в собственном форке — решили не тратить на это время и здесь. Индикатор непрочитанного (без логотипов) закрывает исходный запрос «видно, есть ли нотификации из другого workspace».
...
### Пошаговый план (индикатор + логотипы) — историческая версия, логотипы не выполнялись

Ниже — план, каким он был составлен изначально, до того как индикатор был реализован собственной (не intabia) архитектурой, а логотипы — отменены. Оставлено для истории/контекста, шаги 2-4/7 по логотипам и notifier-сервису не выполнялись, см. «Статус реализации» и подраздел про логотипы выше.
...
- **Backend account-service — только Postgres.** Отдельная Mongo-миграция не нужна (для `has_unread` тем не менее сделали и Mongo-реализацию `AccountDB`, чтобы не ломать типы/интерфейс — но её никто не гоняет в проде).
...
- **Индикатор непрочитанного реализован своей архитектурой**, не через `pulse`/отдельный notifier-сервис/деплой-манифест — see раздел 1 и «Статус реализации». Пункт про «деплой-манифест сразу» из более раннего решения — снят, не понадобился.
- **Логотипы воркспейсов — не портируем.** См. подраздел «Логотипы воркспейсов — решили НЕ портировать» в разделе 1: сама intabia откатила эту фичу у себя же, цена (отдельный публичный файловый эндпоинт в account-service) не стоит результата.
