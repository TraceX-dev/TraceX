...
### Cross-workspace индикатор — реализован своей архитектурой (не intabia-pulse)

По итогам разбора п.1 решили **не** переносить `@intabiafusion/pulse` + отдельный долгоживущий `services/notifications`-сервис (N live-соединений на аккаунт) — слишком тяжело для того, что нужно (просто boolean-флаг), и plана "деплой-манифест сразу" из п.1 не потребовалось. Вместо этого — push-модель через уже существующий account-service:

1. **Хранение** (коммит `c22ba12415`): колонка `workspace_members.has_unread` (Postgres, миграция `v28` — `v26`/`v27` уже заняты другими правками, как и предполагалось в п.1 плана), `AccountDB.setWorkspaceMemberUnread()` (Postgres + Mongo), `getAccountWorkspaces()` отдаёт флаг как `WorkspaceInfoWithStatus.hasUnread`.
2. **RPC** (тот же коммит): `setWorkspaceMemberUnread` в account-service. Авторизация: поднять флаг (`true`) для любого аккаунта или снять чужой — только по сервисному токену (`service: 'notification'`); снять свой собственный — самообслуживание, без привилегий. `AccountClient.setWorkspaceMemberUnread` добавлен в `@hcengineering/account-client`.
3. **Триггер** (тот же коммит): новый `OnInboxNotificationCreate` в `server-plugins/notification-resources` (зарегистрирован в `models/server-notification`), срабатывает на каждое создание `InboxNotification` независимо от push-настроек. Шлёт `hasUnread=true` через `generateToken(systemAccountUuid, workspace, {service:'notification'})` + `getAccountClient()` — по аналогии с внешним вызовом в `server-plugins/calendar-resources`. Best-effort (ошибка только логируется, не блокирует доставку нотификации).
4. **Фронт** (коммит `d0c6ac802a`): `SelectWorkspaceMenu.svelte` рисует точку (`NotifyMarker kind="simple"`) у названия воркспейса, если `ws.hasUnread === true` и это не текущий открытый воркспейс. `Workbench.svelte` — когда локальный инбокс переходит из "есть непрочитанное" в "всё прочитано" (уже существовавшая реактивная проверка `HasInboxNotifications`), вызывает новый `reportWorkspaceRead()` (`workbench-resources/utils.ts`) — самообслуживание, снимает свой же флаг.

Логотипы воркспейсов (вторая часть п.1) — **не начаты**, остаются в бэклоге отдельным пунктом.

Известные ограничения/следующие шаги:
- Живой account-service/Postgres в этой песочнице недоступен — миграция и RPC-путь проверены только `tsc --noEmit` (server/account, foundations/core/packages/core, foundations/core/packages/account-client, server-plugins/notification-resources, server-plugins/notification, models/server-notification, plugins/workbench-resources), не end-to-end.
- Svelte-шаблонные правки (`SelectWorkspaceMenu.svelte`, `Workbench.svelte`) проверены вручную + eslint, полный `svelte-check` не прогнан (та же ограниченность окружения по времени, что и раньше в этой сессии) — рекомендуется прогнать перед мёржем.
- Нет ретро-бэкафилла: `has_unread` для существующих непрочитанных нотификаций (созданных до деплоя миграции) не выставится сам — индикатор начнёт работать только для новых нотификаций после раскатки. Если нужен бэкафилл — отдельная разовая миграция/скрипт.
- Нет rate-limiting/дебаунса на вызовы `setWorkspaceMemberUnread` — при всплеске нотификаций (например массовая рассылка) будет всплеск RPC к account-service, по одному на уникального получателя на батч. Для нормальной нагрузки это не проблема (флаг и так идемпотентен), но стоит понаблюдать в проде.
