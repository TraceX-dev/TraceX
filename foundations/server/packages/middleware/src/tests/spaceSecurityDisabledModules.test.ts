  ...
  for (const key of Object.keys(query ?? {})) {
    ...
    if (cond !== null && typeof cond === 'object' && !Array.isArray(cond)) {
      ...
      if (cond.$nin !== undefined) {
        const excluded: boolean = cond.$nin.includes(val)
...
async function setup (
  ...
  const productsCardId: Ref<Doc> = generateId()
  const otherCardId: Ref<Doc> = generateId()
  const cards = [
    { _id: productsCardId, _class: CARD_CLASS, space: productsSpaceId },
    { _id: otherCardId, _class: CARD_CLASS, space: otherSpaceId }
  ]
  ...
  const next: Middleware = {
    findAll: (async (_ctx: any, _class: any, query: any) => {
      if (_class === core.class.Space) return spaces.filter((sp) => matchesQuery(sp, query)) as any
      if (_class === core.class.ModulePermissionGroup) {
        return moduleGroups.filter((g) => matchesQuery(g, query)) as any
      }
      if (_class === CARD_CLASS) return cards.filter((c) => matchesQuery(c, query)) as any
  ...
  const hierarchy: any = {
    ...
    getDomain: (_class: Ref<Class<Doc>>) => {
      ...
      if (_class === CARD_CLASS) return 'card'
      return 'tx'
    }
  }
...
describe('SpaceSecurityMiddleware – disabled-module exclusion from findAll', () => {
  it('excludes content living in the disabled module space for the matching role', async () => {
    const s = await setup([{ role: AccountRole.Guest, enabled: false, spaceClass: PRODUCTS_SPACE_CLASS }])
    const ctx = makeCtx(makeAccount(AccountRole.Guest, s.ALICE))
    const res = await s.mw.findAll(ctx, CARD_CLASS, {})
    const ids = res.map((r: any) => r._id)
    expect(ids).not.toContain(s.productsCardId)
    expect(ids).toContain(s.otherCardId)
  })
  ...
  it('does not affect a role the disabled group does not target', async () => {
    ...
    expect(ids).toEqual([s.productsCardId, s.otherCardId].sort(byId))
  })
  ...
  it('an enabled module is not excluded', async () => {
    ...
    const ctx = makeCtx(makeAccount(AccountRole.Guest, s.ALICE))
    const res = await s.mw.findAll(ctx, CARD_CLASS, {})
    expect(res.map((r: any) => r._id)).toContain(s.productsCardId)
  })
  ...
  it('also excludes the Space document itself from a direct core.class.Space query', async () => {
    const s = await setup([{ role: AccountRole.Guest, enabled: false, spaceClass: PRODUCTS_SPACE_CLASS }])
    const ctx = makeCtx(makeAccount(AccountRole.Guest, s.ALICE))
    const res = await s.mw.findAll(ctx, core.class.Space, {})
    const ids = res.map((r: any) => r._id)
    expect(ids).not.toContain(s.productsSpaceId)
    expect(ids).toContain(s.otherSpaceId)
  })
})
