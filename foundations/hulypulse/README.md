...
Pre-build docker images is available at: tracex/service_hulypulse:{tag}.

You can use the following command to run the image locally:
```bash
docker run -p 8095:8095 -it --rm tracex/service_hulypulse:{tag}
```
...
```bash
 ...
 docker run --rm -it --network dev_default -p 8095:8095 tracex/service_hulypulse:{tag}
```
